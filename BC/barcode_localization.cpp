//
// Created by pilcq on 2019/10/22.
//
...
void createVectorsOfIntensities(std::vector<int> &support_candidates,
    ...
    for (int i = 0; i < intensities_size; i++) {
        ...
        if (support_candidates_threshold < support_candidates[i]) {
            ...
            for (start = 0; (temp_start_x > lineIterators[5].pos().x) &&
                            (start < lineIterators[5].count); ++lineIterators[5], start++);
            for (end = start;
                 (temp_end_x > lineIterators[5].pos().x) && (end < lineIterators[5].count); ++lineIterators[5], end++);
